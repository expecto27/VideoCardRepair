    ...
    upload(req, res, async (err) => {
      try {
        ...
        await transporter.sendMail({
          ...
          html: `
            <h2>Ваши коды для работы с постаматом</h2>
...
exports.findByUserId = (req, res) => {
    var SQL = `SELECT s.id as "service_id", g.name as "card", r.status, r.updatedAt, s.title, r.createdAt, r.postamat_drop_code, r.postamat_pickup_code, r.photo_path FROM requests r JOIN gpus g ON r.gpu_id = g.id JOIN services s ON r.service_id = s.id JOIN postamats p ON p.id = r.postamat_id WHERE r.user_id = ` + req.params.user_id; 
    console.log(SQL);
    db.sequelize.query(
        SQL, { 
            type: db.sequelize.QueryTypes.SELECT
